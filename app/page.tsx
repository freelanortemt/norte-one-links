...
export default function Home() {
  return (
    <main className="min-h-screen bg-softblack text-premium">
      <div className="mx-auto min-h-screen w-full max-w-[480px] overflow-hidden bg-[radial-gradient(circle_at_50%_-8%,rgba(255,255,255,0.22),transparent_25rem),linear-gradient(160deg,#F6F5F2_0%,#EFECE6_46%,#D9D9D6_100%)] shadow-2xl">
        <section className="relative px-5 pb-7 pt-9 text-center text-softblack">
          ...
          <div className="relative">
            <a
              href="#"
              className="mx-auto grid h-36 w-36 place-items-center rounded-full border border-white/80 bg-[radial-gradient(circle_at_35%_25%,#696970,#111111_55%,#050505)] shadow-[0_22px_70px_rgba(17,17,17,0.32)]"
              aria-label="Norte One"
            >
              <span className="rounded-full border border-white/10 px-5 py-3 text-3xl font-black tracking-normal text-premium">
                N1
              </span>
            </a>

            <p className="mt-5 text-xs font-bold uppercase tracking-[0.28em] text-titanium">
              Norte One
            </p>
            ...
            <a
              href={WHATSAPP_URL}
              className="mt-8 inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-full bg-softblack px-6 text-sm font-bold text-premium shadow-premium transition hover:-translate-y-0.5 hover:bg-graphite"
            >
        ...
        <section className="px-4 pb-5">
          <div className="reveal rounded-[2rem] border border-white/70 bg-white/62 p-5 text-softblack shadow-premium backdrop-blur-xl">
            <div className="mb-5 flex items-center gap-3">
              <span className="grid h-11 w-11 place-items-center rounded-2xl bg-softblack text-premium">
                <Sparkles size={20} />
              </span>
            ...
            <div className="mt-5 grid grid-cols-2 gap-3">
              {proofCards.map((item) => (
                ...
                >
                  <CheckCircle2 className="mb-4 text-softblack" size={20} />
        ...
        <section className="px-4 pb-8">
          <div className="reveal relative overflow-hidden rounded-[2rem] bg-[radial-gradient(circle_at_20%_0%,rgba(255,255,255,0.22),transparent_15rem),linear-gradient(145deg,#2B2B31,#111111)] p-6 text-premium shadow-premium">
            ...
            <a
              href={WHATSAPP_URL}
              className="mt-6 inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-full bg-premium px-6 text-sm font-bold text-softblack transition hover:bg-white"
            >
      ...
      <div className="absolute inset-x-0 bottom-0 p-3">
        <div className="ml-auto w-full rounded-[1.5rem] border border-white/18 bg-softblack/68 p-4 text-left text-premium shadow-glass backdrop-blur-xl sm:w-[78%]">
          <div className="mb-3 flex items-center gap-2">
            <span className="grid h-9 w-9 place-items-center rounded-full bg-premium text-softblack">
              <Icon size={18} />
            </span>
          ...
          <span className="mt-4 inline-flex min-h-10 items-center justify-center gap-2 rounded-full bg-premium px-4 text-xs font-bold text-softblack">
            {card.cta}
            <ArrowRight size={14} />
          </span>
        </div>
